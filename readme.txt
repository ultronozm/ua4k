...
Included New Game
- `drone-swarm.txt`: lockstep dual-drone puzzle with 15 levels. Both drones move on every input; `#` acts as a hard force field that blocks approach without serving as a bump-stall surface; and `k`/`g` gates only open when one drone hits `k` while the other hits `g` on the same move.
